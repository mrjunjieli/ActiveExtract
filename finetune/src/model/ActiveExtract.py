...
from model.utils import PositionalEncoding, PositionalEncodingPermute2D
from model.TalkNet_ASD.talkNetModel import talkNetModel
...
class Cross_Sepformer(nn.Module):
    def __init__(self,
        ...
        self.Separator = Cross_Dual_Path_Model(num_spks=num_spks,
                                               ...
                                               skip_around_intra=True)
        # ASD embedding 
        self.asd = talkNetModel(retrain=True)
        if asd_pretrained_model!=None:
            pretrained_model = torch.load(asd_pretrained_model)
            state = self.asd.state_dict()
            for key in state.keys():
                pretrain_key = 'model.' + key
                if pretrain_key in pretrained_model.keys():
                    state[key] = pretrained_model[pretrain_key]
                else:
                    print("not %s loaded" % pretrain_key)
            self.asd.load_state_dict(state)
            for param in self.asd.parameters():
                param.requires_grad = False
        
        self.fusion = nn.Conv1d(128+256,N_encoder_out*2,1,1)
    ...
    def forward(self, mix,mix_mfcc, video):
        mix_w = self.AudioEncoder(mix) # 1,B,256,11999
        asd_emb,visual_emb = self.asd(mix_mfcc,video)

        asd_emb = asd_emb.permute(0,2,1) # B 256,150
        visual_emb = visual_emb.permute(0,2,1) # B 128,150
        video_emb = self.fusion(torch.cat((asd_emb,visual_emb),1)) # B,512,150

        est_mask = self.Separator(mix_w, video_emb)
        mix_w = torch.stack([mix_w] * self.num_spks)
        sep_h = mix_w * est_mask  # 1,B,256,11999
        ...
        est_source = est_source.permute(0, 2, 1).squeeze(1)
...
def ActiveExtract(kernel_size=16, N_encoder_out=256, num_spks=1,
                 asd_pretrained_model = None):

    InterSeparator = CrossTransformerBlock(num_layers=4,
                                           ...
                                           use_positional_encoding=False,
                                           norm_before=True)
    IntraSeparator = SBTransformerBlock(num_layers=4,
                                        d_model=N_encoder_out,
                                        nhead=8,
                                        d_ffn=1024,
                                        dropout=0,
    ...
    return Cross_Sepformer(IntraSeparator,
                           ...
                           num_spks=num_spks,
                 asd_pretrained_model = asd_pretrained_model)
