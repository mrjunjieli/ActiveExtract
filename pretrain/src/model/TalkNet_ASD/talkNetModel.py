...
from model.TalkNet_ASD.audioEncoder import audioEncoder
from model.TalkNet_ASD.visualEncoder import visualFrontend, visualTCN, visualConv1D
from model.TalkNet_ASD.attentionLayer import attentionLayer
...
class talkNetModel(nn.Module):
    ...
    def forward_audio_visual_backend(self, x1, x2,retrain=False): 
        x = torch.cat((x1,x2), 2)    
    ...
    def forward(self, audioFeature, visualFeature):
        audioEmbed = self.forward_audio_frontend(audioFeature) # feedForward
        ...
        return outsAV,visualEmbed

if __name__ =='__main__':
    model = talkNetModel().cuda()
    # audioFeature = torch.randn(2,600,13).cuda()
    # visualFeature = torch.randn(2,150,112,112).cuda()

    # audioEmbed = model.forward_audio_frontend(audioFeature) # feedForward
    ...
    # outsAV= model.forward_audio_visual_backend(audioEmbed, visualEmbed,retrain=True)  

    # print(outsAV.shape) #
    print("\nTotal number of parameters: {} \n".format(sum(p.numel() for p in model.parameters())/ 10**6)) #15.7M  
    print("\nTotal number of trainable parameters: {} \n".format(sum(p.numel() for p in model.parameters() if p.requires_grad) / 10**6)) # 15.7M
